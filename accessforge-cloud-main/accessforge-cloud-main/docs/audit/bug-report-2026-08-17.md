...
- **RESOLVED 2026-08-18 (`3e21014`) — one root cause, TWO manifestations; do not file them as separate bugs.** The root cause is always *single-day fetch vs duty-attributed trim*. Which symptom appears depends on `cb309d4` (the unified crew-set identity for duty grouping, landed after this report was written):
  - **Pre-`cb309d4`** (as written above): the rider difference *breaks* grouping, the return leg is attributed to day N+1 and trimmed from a day-N fetch → Copilot STEP 4 misses the neighbour → **No where the report says Yes**.
  - **Post-`cb309d4`**: the unified identity *connects* the pair, the return leg is absorbed into day N's duty and trimmed from a day-N+1 fetch → asking about the return leg on its own calendar date reads **"No flight … in the LEON report"** (this is the manifestation the failing test reproduced).
  - The fix covers both: the Heavy-intent fetch spans period ±1 day (zero extra LEON calls — same 2 JSON-RPC requests; the upstream already queries ±2 buffered days) and answer targeting filters by the row's own `date_STD_log_UTC`. Duty grouping's crew set untouched, per the 2026-08-18 ruling. Pinned by `test_heavy_cross_consistency.py::test_case_6_cross_midnight_return_with_rider_difference`, with the test fetcher corrected to trim via the real `select_rows_for_period`.
