...
| `backend/main.py:29-41` | Module import, immediately after `create_all()` | Reflects every current metadata table; for an existing table, adds any missing column with raw `ALTER TABLE ... ADD COLUMN`. It affects every model column that is missing in an already-existing table. | Runtime compatibility DDL | Critical: it silently changes schema on import, is not versioned, catches and suppresses failures, and cannot express indexes, unique constraints, FKs, nullability, server defaults, or data backfills safely. | Replace each proven historical delta with an explicit Alembic revision after parity diagnostics. Delete this loop only after its known effects are represented in migrations and an adopted SQLite copy passes. |
...
| 0.4.2 — explicit migration/bootstrap command | Add `backend/bootstrap.py` and focused tests; update operator documentation only | One explicit command that validates an operator-provided target then runs `alembic upgrade head` for fresh databases. It must not be called from API imports. | Fresh temporary SQLite becomes current/head; invalid or existing un-stamped target is rejected with a clear error; API remains unchanged. | Revert the new command only; stop if it can fall back to `redsea.db` implicitly. | `feat: add explicit database bootstrap command` | 0.4.1. |
| 0.4.3 — adopt/stamp a disposable existing SQLite copy | Add `backend/adopt_database.py` or extend the diagnostic command, plus focused tests and runbook updates | Require backup proof and an all-green parity report before allowing `stamp a4fcbd8f8388` on an explicitly named disposable copy. | Matching copy stamps and becomes `current --check-heads`; mismatches, bad integrity, and unknown revisions fail without modifying the copy. | Restore the copy from backup; stop on any operation against `redsea.db`. | `feat: add guarded sqlite adoption command` | 0.4.1 and 0.4.2. |
| 0.4.4 — retire runtime ALTER TABLE compatibility loop | Modify `backend/main.py`; add/update a focused startup/schema test | Remove only lines implementing the raw `ALTER TABLE ... ADD COLUMN` loop. Preserve `create_all()` temporarily. | Existing adopted copy and fresh migrated temp DB start without ALTER calls; no migration parity regression. | Revert this commit to restore compatibility loop; stop if any known supported database needs the loop to start. | `refactor: retire runtime column migration loop` | 0.4.3 and a validated repair migration for any discovered drift. |
