...
export class App {
  ...
  constructor(container: HTMLElement) {
    ...
    this.renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true }); // added alpha
    this.renderer.autoClear = false;
    this.renderer.setSize(window.innerWidth, window.innerHeight);
    this.renderer.setClearColor(0x000000, 0); // transparent background if needed
    ...
    this.params = {
      numFlights: Math.min(150, MAX_FLIGHTS), // Limit to 150 flights max
      elevationOffset: 15,
      segmentCount: 100,
      planeSize: 100,
    ...
    this.controls.enableZoom = false; // Disable scroll zoom for landing page
    ...
    this.svgTexturePromise = (async () => {
      try {
        ...
        for (const plane of this.planeEntries) {
          ...
          const url = `${import.meta.env.BASE_URL || "/"}assets/planes/${svgPath}`;
